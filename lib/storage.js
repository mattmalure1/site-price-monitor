...
async function addItem({ url, name, selector, currentPrice, targetPrice, alertCondition, percentThreshold, checkIntervalMinutes, fallbackSelectors, useBrowserRendering }) {
  const items = await getAllItems();
  const id = generateId();
  const item = {
    id,
    ...
    useBrowserRendering: useBrowserRendering || false,
    lastChecked: null,
    lastError: null,
    errorCount: 0,
    lastAlertAt: null,
